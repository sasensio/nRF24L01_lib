...
{
	return nRF24_send_CMD(R_REGISTER|reg,0,0,&data_reg,bytes_length);        //CMD: 000A AAAA, where A is the address from  the register
}
...
char nRF24_set_bit(char reg,char bit)  //return the status from the register.
{
	char data;
...
char nRF24_res_bit(char reg,char bit)  //return the status from the register.
{
	char data;
	nRF24_read_reg(reg,&data,1);
	if !TESBIT(data,bit)
...
char nRF24_PD_mode()    //Power Down mode
{
	...
	status=nRF24_read_reg(nRF24_CONFIG_REG,0,0,&data,1);
	if (TESBIT(data,PWR_UP)								//check bits to avoid rewrite
	{
		RESBIT(data,PWR_UP);    
		nRF24__write_reg(nRF24_CONFIG_REG, &data, 1 );
	}
	disableRF24;
	return data;
}	
...
void nRF24_RX_mode()		//Reception mode
{
char data;
char status;
...
void nRF24_SBI_mode()		//Standby I mode
{
	char data;
	char status;
...
void nRF24_SBII_mode()		//StandbyII mode  
{
	char data;
	char status;
...
char setup_EShockBurst(char addressEspaceSize, char ard, char arc, char)
{
	...
	char data=0x00;
	char ret;
	nRF24_write_reg(SETUP_AW_REG,&addressEspaceSize,1);
	
	data= ard<<ARD | arc<<ARC;			//prepare SETUP_RETR
	ret= nRF24_write_reg(SETUP_RETR_REG,&data,1);
	return ret;
}
...
//autoack is must be enable for dynamic payload
{
	...
	else
	{
		...
		nRF24_write_reg(RX_PW_P0+pipeID,&sizePL,1);
		if autoACK
